...
const plotCoord = wire => {
  ...
  const dir = {
    R: (dist, currCoord) => {
      ...
      for (let i = 1; i <= dist; i++) {
        coord.push([currCoord[0] + i, currCoord[1]]);
      }
    },
    L: (dist, currCoord) => {
      x -= dist;
    ...
    U: (dist, currCoord) => {
      y += dist;
      for (let i = 1; i <= dist; i++) {
        coord.push([currCoord[0], currCoord[1] + i]);
      }
    },
    D: (dist, currCoord) => {
      y -= dist;
  ...
  for (let step of wire) {
    result.push([step.slice(0, 1), parseInt(step.slice(1), 10)]);
  }
  for (let step of result) {
    dir[step[0]](step[1], [x, y]);
  }
  return coord; //result is in array of coordinates [[x,y], ...]
};

const findIntersect = ([wire1, wire2]) => {
  console.time("Done in");
  const result = [];
  const wire1Coord = plotCoord(wire1);
  const wire2Coord = plotCoord(wire2);
  for (let coord1 of wire1Coord) {
    for (let coord2 of wire2Coord) {
      coord1[0] === coord2[0] && coord1[1] === coord2[1]
        ? result.push([coord1[0], coord1[1]])
        : null;
    }
  }
  let closest = Math.abs(result[0][0]) + Math.abs(result[0][1]);
  for (let i = 1; i < result.length; i++) {
    //get the closest intersection relative to origin
    closest > Math.abs(result[i][0]) + Math.abs(result[i][1])
      ? (closest = Math.abs(result[i][0]) + Math.abs(result[i][1]))
      : null;
  }
  const mostEfficient = [];
  for (let efficient of result) {
    //finds total steps from both wire to intersect, result in array
    let step = 0;
    for (let i = 0; i < wire1Coord.length; i++) {
      wire1Coord[i][0] === efficient[0] && wire1Coord[i][1] === efficient[1]
        ? (step += i + 1)
        : null;
    }
    for (let i = 0; i < wire2Coord.length; i++) {
      wire2Coord[i][0] === efficient[0] && wire2Coord[i][1] === efficient[1]
        ? (step += i + 1)
        : null;
    }
    mostEfficient.push(step);
  }
  let min = mostEfficient[0];
  for (let i = 1; i < mostEfficient.length; i++) {
    //get the most efficient steps to intersect from origin
    min > mostEfficient[i] ? (min = mostEfficient[i]) : null;
  }
  console.timeEnd("Done in");
  return min; //most efficient intersection counted in steps relative to origin
};

console.log(findIntersect(testWire1));
console.log(findIntersect(testWire2));
console.log(findIntersect([wires1, wires2]));
